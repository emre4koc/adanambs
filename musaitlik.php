...
// --- EKLENEN ZAMAN KONTROL FONKSİYONU BAŞLANGIÇ ---

/**
 * Kullanıcının müsaitlik bildirimi yapabileceği zaman diliminde olup olmadığını kontrol eder.
 * Pazartesi 09:00:00'dan Salı 12:00:00'a kadar izin verir.
 *
 * @return bool İzin veriliyorsa true, aksi halde false.
 */
function is_musaitlik_period_open() {
    // İstanbul (Türkiye) saat dilimini kullanıyoruz, sunucu saatinizin doğru olduğundan emin olun.
    date_default_timezone_set('Europe/Istanbul'); 
    $now = new DateTime();
    $day_of_week = (int)$now->format('w'); // 0 (Pazar) - 6 (Cumartesi)
    $time_of_day = (int)$now->format('Hi'); // Örn: 0900 (09:00), 1200 (12:00)

    // Pazartesi (1) ve Salı (2) günleri için kontrol
    if ($day_of_week == 1) { // Pazartesi
        // Pazartesi 09:00'dan sonra
        return $time_of_day >= 900;
    } elseif ($day_of_week == 2) { // Salı
        // Salı 12:00'dan önce (12:00 dahil)
        return $time_of_day <= 1200;
    }
...
$is_open = is_musaitlik_period_open(); 

// --- EKLENEN ZAMAN KONTROL FONKSİYONU BİTİŞ ---
...
// Talebi geri çekme (ZAMAN KISITLAMASI YOK)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['talep_gonder'])) {
    ...
    // --- POST İSTEĞİ ZAMAN KONTROLÜ BAŞLANGIÇ ---
    if (!$is_open) {
        $_SESSION['mesaj'] = ['tip' => 'error', 'icerik' => 'Müsaitlik değişikliği talebi sadece **Pazartesi 09:00** ile **Salı 12:00** arasında yapılabilir.'];
        header("Location: musaitlik.php");
        exit();
    }
    // --- POST İSTEĞİ ZAMAN KONTROLÜ BİTİŞ ---
...
<div class="container mx-auto">
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <?php if ($is_open): ?>
            <p class="text-sm text-gray-600 mb-4 font-bold text-green-700">✅ DEĞİŞİKLİK YAPILABİLİR: Pazartesi 09:00 ile Salı 12:00 arası.</p>
        <?php else: ?>
            <p class="text-sm text-gray-600 mb-4 font-bold text-red-700">❌ DEĞİŞİKLİK KAPALI: Pazartesi 09:00 ile Salı 12:00 arasında talep gönderebilirsiniz.</p>
        <?php endif; ?>
        ...
        <?php if ($is_open): // Sadece zaman açıkken formu göster ve etkileşimi etkinleştir ?>
        ...
        <?php else: // Zaman kapalıyken sadece okuma amaçlı tabloyu göster ?>
            
            <?php if ($talep): ?>
                <div class="p-4 mb-4 text-sm text-yellow-800 rounded-lg bg-yellow-100">
                    <strong class="block text-base">Beklemede olan bir değişiklik talebiniz var.</strong>
                    <p class="mt-1">Gerekçeniz: <?php echo htmlspecialchars($talep->gerekce); ?></p>
                    <a href="?action=geri_cek&id=<?php echo $talep->id; ?>" 
                       onclick="return confirm('Bu değişiklik talebini geri çekmek istediğinizden emin misiniz?');"
            ...
            <div class="p-4 my-4 text-sm text-blue-800 rounded-lg bg-blue-100">
                <strong class="block text-base">Müsaitlik Bildirimi Kapalı</strong>
                <p class="mt-1">Müsaitlik tablonuzu görüntüleyebilirsiniz, ancak değişiklik talebi sadece **Pazartesi 09:00** ile **Salı 12:00** arasında yapılabilir.</p>
            </div>
                ...
                <table class="min-w-full border">
                    ...
                    <tbody>
                        ...
                        <tr class="text-center">
                            ...
                            <?php foreach (array_keys($zamanlar) as $zaman): ?>
                            <td class="py-2 px-4 border">
                                <?php if (isset($mevcut_musaitlik[$gun][$zaman]) && $mevcut_musaitlik[$gun][$zaman] == 1): ?>
                                    <span title="Müsait" class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-green-100 text-green-600">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                                        </svg>
                                    </span>
                                <?php else: ?>
                                    <span title="Müsait Değil" class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-red-100 text-red-500">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <?php endforeach; ?>
                            <td class="py-2 px-4 border">
                                <textarea disabled rows="1" class="w-full border p-1 rounded text-sm bg-gray-50 text-gray-600"><?php echo htmlspecialchars($mevcut_notlar[$gun] ?? ''); ?></textarea>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
...
<script>
// JavaScript kodu sadece form açıkken çalışmalıdır.
<?php if ($is_open): ?>
