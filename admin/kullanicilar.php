...
@include_once '../lib/SimpleXLSXGen.php'; // Excel oluşturucu kütüphanesi - BU SATIRI EKLE
...
// KULLANICI LİSTESİNİ EXCEL'E AKTARMA İŞLEMİ
if (isset($_GET['action']) && $_GET['action'] == 'export_excel') {
    // Tüm kullanıcı verilerini çek
    $stmt = $pdo->prepare("SELECT id, ad, soyad, email, rol, klasman, telefon, lisans_no, dogum_tarihi, tc_no, banka_bilgisi, iban, dogum_yeri, kan_grubu, il, ilce FROM users");
    $stmt->execute();
    ...
    if (class_exists('Shuchkin\SimpleXLSXGen')) {
        try {
            ...
            // Başlık satırı
            $excel_data[] = [
                'ID', 'Ad', 'Soyad', 'E-posta', 'Rol', 'Klasman', 'Telefon', 'Lisans No', 
                'Doğum Tarihi', 'T.C. No', 'Banka Bilgisi', 'IBAN', 'Doğum Yeri', 
                'Kan Grubu', 'İl', 'İlçe'
            ];
            
            foreach ($users_data as $row) {
                $row['rol'] = getRoleName($row['rol']); // Rol ID'sini isme çevir
                
                // Tarih formatını düzelt
                if ($row['dogum_tarihi'] && $row['dogum_tarihi'] != '0000-00-00') {
                    $row['dogum_tarihi'] = date('d.m.Y', strtotime($row['dogum_tarihi']));
                } else {
                    $row['dogum_tarihi'] = '';
                }
            ...
            $dosya_adi = 'kullanicilar_' . date('Y-m-d_His') . '.xlsx';
...
if(isset($_POST['kullanici_ekle'])) {
    ...
    $banka_bilgisi = trim($_POST['banka_bilgisi']);
    ...
    $stmt = $pdo->prepare("INSERT INTO users (ad, soyad, email, sifre, rol, klasman, telefon, lisans_no, dogum_tarihi, tc_no, banka_bilgisi, iban, dogum_yeri, kan_grubu, il, ilce) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if($stmt->execute([$ad, $soyad, $email, $sifre, $rol, $klasman, $telefon, $lisans_no, $dogum_tarihi, $tc_no, $banka_bilgisi, $iban, $dogum_yeri, $kan_grubu, $il, $ilce])) {
...
if (isset($_POST['toplu_kullanici_yukle']) && isset($_FILES['csv_dosyasi']) && $_FILES['csv_dosyasi']['error'] == 0) {
    ...
    fgetcsv($file, 1000, ";"); // Başlık satırını atla
    ...
    while (($line = fgetcsv($file, 1000, ";")) !== FALSE) {
        try {
            ...
            $stmt->execute([
                ...
                trim($line[9]), trim($line[10]), trim($line[11]),
                trim($line[12]), trim($line[13]), trim($line[14]), trim($line[15])
            ]);
            $eklenen_sayisi++;
...
<div class="container mx-auto">
    <div class="bg-white p-6 rounded-lg shadow-md mb-6">
        ...
        <?php endif; ?>
        <p class="text-sm text-gray-600">Sütunlar: Ad, Soyad, E-posta, Şifre, Rol (2:Hakem, 3:Gözlemci), Klasman, Telefon, Lisans No, Doğum Tarihi (GG.AA.YYYY), T.C. No, Banka Bilgisi, IBAN, Doğum Yeri, Kan Grubu, İl, İlçe</p>
        <a href="data:text/csv;charset=utf-8,%EF%BB%BFAd;Soyad;E-posta;Şifre;Rol;Klasman;Telefon;Lisans No;Doğum Tarihi;T.C. No;Banka Bilgisi;IBAN;Doğum Yeri;Kan Grubu;İl;İlçe%0AAli;Veli;ali.veli@hakem.com;sifre123;2;İl Hakemi;05551234567;12345;15.03.1990;12345678901;Garanti Bankası;TR123456789012345678901234;Ankara;A+;Ankara;Çankaya%0AZeynep;Güneş;zeynep.gunes@gozlemci.com;sifre456;3;Bölgesel Gözlemci;05559876543;67890;20.07.1985;12345678902;İş Bankası;TR123456789012345678901235;İzmir;0-;İzmir;Bornova" download="ornek_kullanici_sablonu.csv" class="text-sm text-blue-600 hover:underline my-2 inline-block"><i class="fas fa-download mr-1"></i>Örnek Şablonu İndir</a>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mb-6">
        ...
        <form method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <input type="text" name="tc_no" placeholder="T.C. Kimlik Numarası" class="border p-2 rounded" maxlength="11">
            <input type="text" name="banka_bilgisi" placeholder="Banka Bilgisi" class="border p-2 rounded">
            <input type="text" name="iban" placeholder="IBAN" class="border p-2 rounded">
            <input type="text" name="dogum_yeri" placeholder="Doğum Yeri" class="border p-2 rounded">
            <input type="text" name="kan_grubu" placeholder="Kan Grubu" class="border p-2 rounded">
            <input type="text" name="il" placeholder="İl" class="border p-2 rounded">
            <input type="text" name="ilce" placeholder="İlçe" class="border p-2 rounded">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
            ...
            <form method="GET" class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                ...
                <a href="?action=export_excel" class="bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700 text-center">Excel'e Aktar</a>
            </form>
        </div>
            ...
            <table class="min-w-full bg-white">
                ...
                <tbody>
                    <?php if(empty($kullanicilar)): ?>
                        <tr><td colspan="5" class="text-center py-4">Filtreye uygun kullanıcı bulunamadı.</td></tr>
                    <?php else: ?>
